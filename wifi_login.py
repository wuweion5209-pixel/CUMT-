...
FORBIDDEN_END   = datetime.time(7, 30)
# ===============================================
...
def is_forbidden_time():
    ...
    t  = now.time()
    wd = now.weekday()  # Monday=0 ... Friday=4, Saturday=5, Sunday=6
    if t >= FORBIDDEN_START:
        # 周五(4)、周六(5)晚上不禁用
        return wd not in (4, 5)
    elif t < FORBIDDEN_END:
        # 周六(5)、周日(6)早上不禁用
...
def wait_until_allowed():
    print("[等待] 当前处于禁止时段 (工作日 23:30-07:30)，等待解禁后自动登录...")
    while is_forbidden_time():
        ...
        print(f"  距 07:30 还有约 {wait_min} 分钟，休眠中...")
